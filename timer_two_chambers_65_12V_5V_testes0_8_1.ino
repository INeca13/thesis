...
//ROTATARY ENCODER PINS
#define ENCODER_CLK 27
#define ENCODER_DT 25
#define ENCODER_SW 23


  QC2Control quickCharge(11, 13); // GREEN AND YELLOW PB WIRE  (SEE DOCUMENTATION)
...
int battery = 52; //check if socket or power bank
int source = 0;


int buttonStartStop = 49; //assay end button
int buttonReset = 47; //reset button
int buttonMinutesUp = 53;//+1 minute
int buttonMinutesDown = 51;//-1 minute
int chamber_1 = 40;//select chamber 1
int chamber_2 = 42;//select chamber 2

int led1Pin = 37; //chamber 1 end
int led2Pin = 36; //chamber 2 end
int led3Pin = 35; //chamber 1 green light
int led4Pin = 34; //chamber 1 red light
int led5Pin = 33; //chamber 2 green light 
int led6Pin = 32; //chamber 2 red light

int buzzerPin1 = 50;//chamber 1 buzzer
int buzzerPin2 = 48;//chamber 2 buzzer
...
int ThermistorPin = A7; //SENSOR CHAMBER 1  A7
int ThermistorPin_2 = A15; //SENSOR CHAMBER 2 A15
int Vo;
int Vo_2;
float R1 = 10000;
...
#define heater_1 9 //CHAMBER 1 HEATING ELEMENT
#define heater_2 7 //CHAMBER 2 HEATING ELEMENT
...
int power_2 = 0;
unsigned long teste;
unsigned long tempo_teste = 10000;

unsigned long t_voltage;
unsigned long tempo_voltage = 5000;
...
void loop() {
  if (millis() - t_voltage <= tempo_voltage){
...
void heating() {
  //ability to change the power delivered to the heating elements with diferent values, regarding if it is powered by a power bank or a socket
  source = digitalRead(battery);
  if (source == 1) { 
    if (ligar == 1){
      analogWrite(heater_2, 0);
      if (average < 68 && average >= 65 && off_1 == 0) {
        power_1 = 200;
        analogWrite(heater_1, power_1);
        t_ligar_1 = millis(); 
      }
      if (average < 65 && off_1 == 0) {
        power_1 = 255;
        analogWrite(heater_1, power_1);
        t_ligar_1 = millis();
      } else if (average >= 68) {
    ...
    if (ligar == 2){
      ...
      if (average_2 < 69 && average_2 >= 67 && off_2 == 0) {
        power_2 = 200;
        analogWrite(heater_2, power_2);
        t_ligar_2 = millis(); 
      }
      if (average_2 < 67 && off_2 == 0) { //(average_2 < 65.5 && off_1 == 0)
      power_2 = 255;
      analogWrite(heater_2, power_2);
      t_ligar_2 = millis();
      } else if (average_2 >= 69) {
        off_2 = 1;
      }
...
void timer() {
  ...
  if (millis()-t_check >= tempo_check){ //check if buttons are pressed
    ...
    if (reset == HIGH && chamber == 1 && flag_1 == 0 && flag_2 == 0 && flag_both == 0){ //when pressed, it will reset all the variables and display the inicial screen
      power_1 = 0;
      analogWrite(heater_1, power_1);
      options = 4; //  print paused
      tempo_target = 0;
      time_running = 0;
      minutes = 0;
      t_reset = millis();
      //Turn of heating element on chamber 1
      if (ligar == 3){
        ligar = 2;
      } else if (ligar == 1){
        ligar = 0;
...
void encoder() {
  ...
  if (button_state == LOW && last_button_state == HIGH && chamber == 1) {
    ...
    if (time_running == 0 && average >= 65){ // after chosing the time, the first press will just start the timer
      time_running = 1;
      options = 2;
      t_target = millis();
    ...
    } else if (time_running == 0 && average < 65) {
      extra_1 = false;
      options = 10;
      t_back = millis();
      display.clearDisplay();
  ...
  if (button_state == LOW && last_button_state == HIGH && chamber == 2) {
    ...
    if (time_running_2 == 0 && average_2 >= 65){ // after chosing the time, the first press will just start the timer
      time_running_2 = 1;
      options = 2;
      t_target_2 = millis();
    ...
    } else if (time_running_2 == 0 && average_2 < 65){
      extra_2 = false;
      options = 10;
      t_back = millis();
      display.clearDisplay();
...
void termistor_1() {
    if (millis() - t_sensor_1 >= tempo_sensor_1){
        ...
        inside = average*0.963;                 //1.104;----------------------------------------------------------------------------------------------------------------------

        if (average >= 65 && extra_1 == false){
          t_extra_1 = millis();
          extra_1 = true;
...
void termistor_2() {
    if (millis() - t_sensor_2 >= tempo_sensor_2){
        ...
        //if (average_2> 30){
        //average_2 = average;//
        inside_2 = average_2*0.95;               //1.104;----------------------------------------------------------------------------------------------------
        if (average_2 >= 66 && extra_2 == false){
          t_extra_2 = millis();
          extra_2 = true;
