...
void termistor_1() {
    if (millis() - t_sensor_1 >= tempo_sensor_1){
        ...
        // print the average~
        if (ligar == 1 || ligar == 3) {
          if (millis() - t_extra_1 >= tempo_extra_1){   //option of only putting the time, since the temperature readings can be oscilating because of the socket power
            digitalWrite(led3Pin, HIGH);
            digitalWrite(led4Pin, LOW);
          } else if (millis() - t_extra_1 < tempo_extra_1){    //option of only putting the time, since the temperature readings can be oscilating because of the socket power
...
void termistor_2() {
    if (millis() - t_sensor_2 >= tempo_sensor_2){
        ...
        if (average_2 >= 72 && extra_2 == false){
          ...
          if (ligar == 2 || ligar == 3) {
            if (millis() - t_extra_2 >= tempo_extra_2){  //option of only putting the time, since the temperature readings can be oscilating because of the socket power
            digitalWrite(led5Pin, HIGH);
            digitalWrite(led6Pin, LOW);
          } else if (millis() - t_extra_2 < tempo_extra_2){      //option of only putting the time, since the temperature readings can be oscilating because of the socket power
