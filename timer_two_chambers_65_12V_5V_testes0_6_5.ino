...
void heating() {
  ...
  if (source == 1) { // quando ligado a corrente, vai funcionar as duas camaras
    if (ligar == 1){
      ...
      if (average < 70 && average >= 67 && off_1 == 0) {
        power_1 = 200;
        analogWrite(heater_1, power_1);
        t_ligar_1 = millis(); 
      }
      if (average < 67 && off_1 == 0) {
        power_1 = 255;
        analogWrite(heater_1, power_1);
        t_ligar_1 = millis();
      } else if (average >= 70) {
    ...
    if (ligar == 2){
      ...
      if (average_2 < 70 && average_2 >= 67 && off_2 == 0) {
        power_2 = 200;
...
void encoder() {
  ...
  if (button_state == LOW && last_button_state == HIGH && chamber == 1) {
    ...
    if (time_running == 0 && average >= 68){ // after chosing the time, the first press will just start the timer
      time_running = 1;
      options = 2;
      t_target = millis();
    ...
    } else if (time_running == 0 && average < 68) {
      extra_1 = false;
      options = 10;
      t_back = millis();
      display.clearDisplay();
  ...
  if (button_state == LOW && last_button_state == HIGH && chamber == 2) {
    ...
    if (time_running_2 == 0 && average_2 >= 68){ // after chosing the time, the first press will just start the timer
      time_running_2 = 1;
      options = 2;
      t_target_2 = millis();
    ...
    } else if (time_running_2 == 0 && average_2 < 68){
      extra_2 = false;
      options = 10;
      t_back = millis();
      display.clearDisplay();
...
void termistor_1() {
    if (millis() - t_sensor_1 >= tempo_sensor_1){
        ...
        inside = average*0.936;                 //1.104;----------------------------------------------------------------------------------------------------------------------

        if (average >= 68 && extra_1 == false){
          t_extra_1 = millis();
          extra_1 = true;
...
void termistor_2() {
    if (millis() - t_sensor_2 >= tempo_sensor_2){
        ...
        //if (average_2> 30){
        //average_2 = average;//
        inside_2 = average_2*0.936;               //1.104;----------------------------------------------------------------------------------------------------
        if (average_2 >= 68 && extra_2 == false){
          t_extra_2 = millis();
          extra_2 = true;
